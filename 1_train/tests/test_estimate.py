...
# Test tensors
INPUT = MyTensor(0, "x0", [1, 32, 32, 4], "int8", False)
WEIGHTS_1 = MyTensor(1, "w1", [1, 2, 2, 8 * 4], "int8", True)
BIAS_1 = MyTensor(2, "b1", [8], "int32", True)
INTERMEDIATE_1 = MyTensor(3, "x1", [1, 32, 32, 32], "int8", False)
WEIGHTS_2 = MyTensor(4, "w2", [16, 3, 3, 32], "int8", True)
...
def test_estimate_rom():
    ...
    # Single layer
    assert estimate_rom([INPUT, WEIGHTS_1, BIAS_1, INTERMEDIATE_1]) == 160
    assert estimate_rom([INTERMEDIATE_1, WEIGHTS_2, BIAS_2, INTERMEDIATE_2]) == 4672
...
def test_estimate_ram():
    ...
    # Multi layer
    assert estimate_ram(ALL_TENSORS, ALL_LAYERS) == 36864
